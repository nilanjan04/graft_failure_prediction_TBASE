...
select  p.PatientID ,
	    ...
	    , datediff(dd,t.datum,
	    			case when datediff(dd,t.datum,t.TPV_Datum) between (365*2) and (365*6) then tpv_datum
	    				when datediff(dd,t.datum,p.Todesdatum ) between (365*2) and (365*6) then p.todesdatum
	    				else CURRENT_TIMESTAMP end) as tenure
	    --Label: Long-term
		--Deaths and transplant failure between 24 and 72 months after transplantation are considered as transplant failure; Everything else as success
		, case when datediff(dd,t.datum,t.TPV_Datum ) between (365*2) and (365*6) or datediff(dd,t.datum,p.Todesdatum ) between (365*2) and (365*6) then 1 else 0 end as Longterm_TransplantOutcome -- 0: Success; 1: Transplant Failure 	
