...
#Change to the number of test samples when test_features are oversampled
...
model = Sequential()
model.add(LSTM(120, input_shape=(train_features.shape[1:]), activation='softsign', return_sequences=True))
model.add(Dropout(0.2))
model.add(LSTM(120, activation='softsign')) 
model.add(Dropout(0.2))
...
#You may check the probablities to decide the cutoff by executing the lines below
#df_predictions = df_predictions.join(df_labels)
#df_predictions.to_csv(r'T:\tbase\short_lstm\prediction_labels.csv')
df_predictions.loc[df_predictions['prediction'] >= 0.0001, ['prediction']] = 1
df_predictions.loc[df_predictions['prediction'] < 0.0001, ['prediction']] = 0
...
# Please change the path accordingly
df_error_analysis.to_csv(r'T:\tbase\short_lstm\Qualitative_res.csv')  #Please change the path accordingly
# =============================================================================
